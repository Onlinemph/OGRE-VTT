 ...
 *
 * Battles resolve *outside* this engine — in this app for the ground half, in
 * Triplanetary-VTT for the space half — and come back through `reportBattle`
 * as a `BattleResult`. The engine cannot verify a result's die rolls (that is
 * what the replay in the result is for); it does verify that the result
 * answers the battle it asked for.
 */

import { type CommandResult, fail, ok } from '@engine/commands.js';
import { type RngState, createRng, nextInt } from '@engine/rng.js';
...
const drawSeed = (state: CampaignState): { state: CampaignState; seed: number } => {
  const draw = nextInt(state.rng, 0x7fffffff);
  return { state: { ...state, rng: draw.state }, seed: draw.value + 1 };
};
    ...
    case 'intercept': {
      ...
      next = log(
        ...
        `${CAMPAIGN_SIDES[cmd.by].name} comes out to meet the convoy off ` +
          `${siteDef(pending.site)?.name ?? pending.site}. Fight the transfer in Triplanetary.`,
      );
      return { state: next, result: ok() };
    }
