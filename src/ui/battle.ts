/**
 * The battle view: panels, pointer and keyboard bindings, and the one-way loop
 *
 ...
 * go, what a shot is worth, whether a ram is allowed — is asked of the engine
 * (`reachable`, `previewAttack`, `canRam`), and every change leaves as a
 * `Command`. It mounts into a host element the shell provides, owns its own
 * listeners for exactly as long as it is mounted, and plays any scenario on
 * the table. A campaign battle ends with a result token for the war room in
 * the companion app; a printed scenario ends with the verdict alone.
 *
 ...
 *
 * This file is shared, character for character in its game half, with the
 * companion Triplanetary app's embedded view; only the chrome around it
 * differs.
 */
 ...
 * must go somewhere, and here that is a token for the war room in the
 * companion app — while a printed scenario is just a game: it ends with a
 * verdict and the door.
 */
export type OgreBattleSource =
  | { readonly kind: 'order'; readonly order: OrderOfBattle }
  | {
      readonly kind: 'scenario';
      /** One of the scenario table's ids; an unknown id falls back to the first. */
      readonly id: string;
      readonly seed: number;
    };

export interface OgreBattleOptions {
  /** Where to mount. The view owns the host's children while the battle is open. */
  readonly host: HTMLElement;
  /** The wiring: scenario table, session and renderer factories, the campaign glue. */
...
export const createOgreBattle = (opts: OgreBattleOptions): OgreBattle => {
  const root = opts.host;
  const deps = opts.deps;

  const battle = opts.battle;
  // An order names its own scenario — The Landing for the campaign, The
  // Assault for Orbital Drop — and an unknown name falls back to the first
  // on the table rather than crashing a battle that already has its forces.
  const byId = (id: string): (typeof deps.scenarios)[number] =>
    deps.scenarios.find((s) => s.id === id) ?? deps.scenarios[0]!;
  const scenario = battle.kind === 'order' ? byId(battle.order.scenarioId) : byId(battle.id);
  const withSetup = opts.setup ?? true;
  const session: SessionPort = deps.createSession(
    scenario.id,
    battle.kind === 'order'
      ? deps.buildScenario(scenario.id, {
          seed: battle.order.seed,
          order: battle.order,
          setup: withSetup,
        })
      : deps.buildScenario(scenario.id, { seed: battle.seed, setup: withSetup }),
  );
  ...
  const renderTopbar = (state: GameState): void => {
    ...
    setChildren(
      ...
      el(
        ...
        button(
          leaveArmed && !state.victory ? 'Really leave?' : 'New game',
          () => {
  ...
  const renderVictory = (state: GameState): void => {
    ...
    actions.push(
      button('New game', () => opts.onExit(), { class: actions.length > 0 ? '' : 'primary' }),
    );
